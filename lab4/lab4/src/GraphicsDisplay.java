...
public class GraphicsDisplay extends JPanel {
    ...
    protected void paintMarkers(Graphics2D canvas) {
        ...
        Double sr_Y = 0.0;
        for (int i = 0; i < graphicsData.length; i++) {
            sr_Y += graphicsData[i][1];
        }
        sr_Y /= graphicsData.length;
        System.out.println(sr_Y);
        for (int i = 0; i < graphicsData.length; i++) {
            ...
            Line2D.Double diagonalLine_1 = new Line2D.Double();
            diagonalLine_1.setLine(

                    constructPoint(xyToPoint(point[0], point[1]), -5, 5),
                    constructPoint(xyToPoint(point[0], point[1]), 5, -5)
            );

            Line2D.Double diagonalLine_2 = new Line2D.Double();
            diagonalLine_2.setLine(
                    constructPoint(xyToPoint(point[0], point[1]), 5, 5),
                    constructPoint(xyToPoint(point[0], point[1]), -5, -5)
            );
            ...
            canvas.draw(diagonalLine_1);
            canvas.draw(diagonalLine_2);
        }
    }
